...
df1 = pd.read_csv(f'outputs_complex/output-{NAME_DF1}.csv')  
df2 = pd.read_csv(f'outputs_complex/output-{NAME_DF2}.csv')  

df_result = pd.DataFrame()  
df_result['Complexity Measure'] = df1['Complexity Measure']  
df_result['Value'] = df1['Value'] - df2['Value']  

df_result.to_csv(f'outputs_diff/{NAME_DF1}-{NAME_DF2}-diff.csv', index=False)
