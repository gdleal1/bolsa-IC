...
df1 = pd.read_csv(f'outputs_complex/output-{NAME_DF1}.csv').head(14)  
df2 = pd.read_csv(f'outputs_diff/{NAME_DF2}-{NAME_DF1}-diff.csv').head(14)  

df_result = pd.DataFrame()  
df_result['Complexity Measure'] = df1['Complexity Measure']  
df_result['Increase (%)'] = df2['Value'] / df1['Value'] * 100

df_result.to_csv(f'outputs_increase/{NAME_DF2}-{NAME_DF1}-increase.csv', index=False)
